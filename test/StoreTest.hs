...
type ValidateSingleTestCase =
  ( MigrationMap
  , Migration
  , [MapValidationError]
  )

type ValidateMigrationMapTestCase =
  ( MigrationMap
  , [MapValidationError]
  )
...
validateSingleTestCases =
  [
    ( emptyMap
    , withDeps
    ,
      [ DependencyReferenceError (mId withDeps) "one"
...
validateMapTestCases =
  ...
  ,
    ( map3
    ,
      [ DependencyReferenceError (mId m4) "one"
