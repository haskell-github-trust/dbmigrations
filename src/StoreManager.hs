...
handleEvent (EvResize w h) = do
  ...
  modify
    ( \appst ->
        appst
          { appMigrationList = (appMigrationList appst) {scrollWindowSize = wSize}
          }
    )
  continue
...
editCurrentMigration = do
  ...
  withTempFile $ \tempHandle tempPath ->
    liftIO $ do
      ...
      let
        editor = maybe "vi" id $ lookup "EDITOR" currentEnv
        spawnEditor = do
...
getSelectedMigration appst = fromJust $ Map.lookup (fst $ getSelected list) mMap
 where
  mMap = storeDataMapping $ appStoreData appst
...
buildUi appst =
  let
    header =
      text titleAttr (" " ++ (storePath $ appStore appst) ++ " ")
        <++> hFill titleAttr '-' 1
        <++> text titleAttr " Store Manager "
    status =
      text bodyAttr $ maybe "<no description>" id $ mDesc $ getSelectedMigration appst
    helpBar =
      text titleAttr "q:quit e:edit "
        <++> hFill titleAttr '-' 1
  in
    header
...
mkState fsStore = do
  ...
  let
    migrationList = mkSimpleList bodyAttr selAttr (appropriateListWindow sz) migrationNames
